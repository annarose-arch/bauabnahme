...
export default function Dashboard({ session, onLogout, onNavigate, isDemo = false }) {
  ...
  const fetchCustomers = async () => { if(!userId) return []; const {data} = await supabase.from("customers").select("*").eq("user_id",userId).neq("phone","__geloescht__").order("name",{ascending:true}); setCustomers(data||[]); return data||[]; };
