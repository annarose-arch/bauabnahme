...
const c=customers.find(x=>String(x.id)===String(inv.customerId)); if(c){ setSelectedCustomer(c); setOpenedReport(null); setEditingReport(null); setView("customers"); setMobileSidebarOpen(false); } else { setEditingInvoice(null); goTo("invoices"); } showNotice("Rechnung gespeichert."); };
  ...
  const navItems = [{key:"home",label:"Start"},{key:"customers",label:"Kunden"},...(isAdmin?[{key:"catalog",label:"Katalog"}]:[]),{key:"new-report",label:"Neuer Rapport"},{key:"reports",label:"Offene Rapporte"},{key:"invoices",label:"Rechnungen"},{key:"trash",label:"Papierkorb"},...(isAdmin?[{key:"settings",label:"Einstellungen"}]:[])];
