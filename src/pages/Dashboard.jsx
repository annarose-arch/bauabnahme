...
    }
  ...
  const deleteCustomer = async (c) => { if(!window.confirm("Löschen?")) return; await supabase.from("customers").update({phone:"__geloescht__"}).eq("id",c.id); setCustomers(p=>p.filter(x=>x.id!==c.id)); showNotice("Kunde in Papierkorb verschoben."); };
