...
    }, [userId, isDemo]);
  ...
  const fetchCustomers = async () => { if(!userId){ return []; } const session = await supabase.auth.getSession(); if(!session.data.session){ showNotice("Session fehlt!"); return []; } const {data, error} = await supabase.from("customers").select("*").eq("user_id",userId).order("name",{ascending:true}); if(error){showNotice("Ladefehler: "+error.message);return[];} setCustomers((data||[]).filter(c=>c.phone!=="__geloescht__")); setTrashCustomers((data||[]).filter(c=>c.phone==="__geloescht__")); return (data||[]).filter(c=>c.phone!=="__geloescht__"); };
