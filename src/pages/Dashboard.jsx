...
export default function Dashboard({ session, onLogout, onNavigate, isDemo = false }) {
  ...
  const onSaveInvoice = (inv) => { saveInvoiceToStorage(inv); const c=customers.find(x=>String(x.id)===String(inv.customerId)); if(c){ setSelectedCustomer(c); goTo("customers"); } else { setEditingInvoice(null); goTo("invoices"); } showNotice("Rechnung gespeichert."); };
