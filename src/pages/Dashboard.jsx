...
  }, [userId, isDemo]);
  ...
  const fetchCustomers = async () => { if(!userId){ return []; } const {data, error} = await supabase.from("customers").select("*").eq("user_id",userId).order("name",{ascending:true}); if(error){showNotice("Ladefehler: "+error.message);return[];} setCustomers((data||[]).filter(c=>c.phone!=="__geloescht__")); setTrashCustomers((data||[]).filter(c=>c.phone==="__geloescht__")); return (data||[]).filter(c=>c.phone!=="__geloescht__"); };
  ...
  const saveCatalog = async (u) => { setCatalog(u); if(!userId) return; const emps = u.employees||[]; const mats = u.materials||[]; const empIds = emps.map(e=>String(e.id)).filter(Boolean); const matIds = mats.map(m=>String(m.id)).filter(Boolean); if(empIds.length>0){ await supabase.from("staff").delete().eq("user_id",userId).not("id","in",`(${empIds.join(",")})`); } else { await supabase.from("staff").delete().eq("user_id",userId); } if(matIds.length>0){ await supabase.from("materials").delete().eq("user_id",userId).not("id","in",`(${matIds.join(",")})`); } else { await supabase.from("materials").delete().eq("user_id",userId); } for(const e of emps){ await supabase.from("staff").upsert({id:e.id,user_id:userId,name:e.name||"",description:e.role||"",rate:Number(e.rate||0)}); } for(const m of mats){ await supabase.from("materials").upsert({id:m.id,user_id:userId,name:m.name||"",description:m.description||"",unit:m.unit||"St",price:Number(m.price||0)}); } };
...
const c=customers.find(x=>String(x.id)===String(inv.customerId)); if(c){ setSelectedCustomer(c); setOpenedReport(null); setEditingReport(null); setView("customers"); setMobileSidebarOpen(false); } else { setEditingInvoice(null); goTo("invoices"); } showNotice("Rechnung gespeichert."); };
  const reopenInvoice = (inv, existingWin) => { console.log("reopenInvoice called", existingWin ? "with win" : "no win"); const win=existingWin||window.open("","_blank","width=980,height=860"); if(!win) { console.log("no window"); return; } const{firmName,firmLogo,firmContact,firmAddress,firmPhone,firmEmail}=getFirmMeta(); const firmDetails=[firmContact&&firmName?firmContact:"",firmAddress,firmPhone,firmEmail].filter(Boolean).join("<br/>"); const p=inv.reportData||{}; win.document.write(buildRechnungHtml({invoiceNr:inv.invoiceNr,firmName,firmLogo,firmContact,firmAddress,firmDetails,name:inv.customer||"-",custAddr:"",custStreet:p.address||"",custZip:p.zip||"",custCity:p.city||"",validWork:(p.workRows||[]).filter(r=>r.employee||toNum(r.hours)>0),validMat:(p.materialRows||[]).filter(r=>r.name||toNum(r.qty)>0),costs:p.costs||{},subtotal:Number(inv.subtotal||p.totals?.subtotal||0),discountPct:Number(inv.discount||0),discountAmt:Number(inv.discountAmt||0),subtotalAfterDiscount:Number(inv.subtotal||p.totals?.subtotal||0)-Number(inv.discountAmt||0),vat:Number(inv.vat||p.totals?.vat||0),totalAmount:Number(inv.totalAmount||0),skontoPct:Number(inv.skontoPct||0),skontoAmt:Number(inv.skontoAmt||0),payDays:Number(inv.paymentDays||30),skontoDays:Number(inv.skontoDays||10),dueDate:"-",skontoDueDate:"-",qrUrl:"",isPro:localStorage.getItem("bauabnahme_plan")==="pro",isDemoMode:!userId,reportDate:inv.date,projectName:p.projectName,rapportNr:p.rapportNr||"",custEmail:(p.customerEmail||customers.find(c=>String(c.id)===String(inv.customerId))?.email||"").trim(),language:uiLanguage})); win.document.close(); };
  const tr = useTranslation(uiLanguage); const navItems = [{key:"home",label:tr.nav.home},{key:"customers",label:tr.nav.customers},...(isAdmin?[{key:"catalog",label:tr.nav.catalog}]:[]),{key:"new-report",label:tr.nav.newReport},{key:"reports",label:tr.nav.reports},{key:"invoices",label:tr.nav.invoices},{key:"trash",label:tr.nav.trash},...(isAdmin?[{key:"settings",label:tr.nav.settings}]:[])];
    ...
    <div style={{minHeight:"100vh",background:BG,color:TEXT,fontFamily:"Inter,system-ui,sans-serif"}}>
      ...
      <div className="dash-grid" style={{display:"grid",gridTemplateColumns:"240px 1fr",minHeight:"100vh"}}>
        ...
        <main style={{padding: window.innerWidth < 600 ? "12px 10px" : 20, minWidth:0, maxWidth:"100%", overflowX:"hidden"}}>
          ...
          <NoticeBanner message={notice}/>{customers.length === 0 && userId && !isDemo && <div style={{textAlign:"center",padding:"8px",background:"rgba(212,168,83,0.1)",margin:"8px 0",borderRadius:8}}><button type="button" onClick={()=>{fetchCustomers().then(c=>fetchProjects(c));fetchReports();fetchInvoices();fetchCatalog();}} style={{background:"#d4a853",color:"#111",border:"none",borderRadius:6,padding:"6px 16px",fontWeight:700,cursor:"pointer",fontSize:13}}>Daten laden</button></div>}
