...
    }, [userId, isDemo]);
  ...
  const fetchCustomers = async () => { showNotice("fetchCustomers userId="+String(userId).slice(0,8)); if(!userId){ return []; } const {data, error} = await supabase.from("customers").select("*").eq("user_id",userId).order("name",{ascending:true}); if(error){showNotice("Ladefehler: "+error.message);return[];} showNotice("Gefunden: "+(data||[]).length+" Kunden"); setCustomers((data||[]).filter(c=>c.phone!=="__geloescht__")); setTrashCustomers((data||[]).filter(c=>c.phone==="__geloescht__")); return (data||[]).filter(c=>c.phone!=="__geloescht__"); };
