...
  }, [userId, isDemo]);
  ...
  const fetchUserSettings = async () => { const {data} = await supabase.from("user_settings").select("*").eq("user_id",effectiveUserId).single(); if(data){ setNextRapportNrState(data.next_rapport_nr); setNextInvoiceNrState(data.next_invoice_nr); localStorage.setItem("bauabnahme_next_rapport_nr", String(data.next_rapport_nr)); localStorage.setItem("bauabnahme_next_invoice_nr", String(data.next_invoice_nr)); localStorage.setItem("bauabnahme_next_customer_nr", String(data.next_customer_nr)); } };
  useEffect(() => {
