...
if (!isDemo && userId) {
  ...
  const fetchInvoices = async () => { 
    if(!userId){ return; }
    const {data} = await supabase.from("invoices").select("*").order("id",{ascending:false}).limit(50);
