...
export function buildRechnungHtml({ language = "DE",
 ...
 const wHtml = validWork.map(r => `<tr><td>${r.employee || "-"}</td><td style="text-align:center">${r.from || "-"}–${r.to || "-"}</td><td style="text-align:right;font-family:monospace">${Number(r.hours || 0).toLocaleString('de-CH',{minimumFractionDigits:2,maximumFractionDigits:2})} h</td><td style="text-align:right;color:#333;width:35px">CHF</td><td style="text-align:right;font-family:monospace;width:90px">${Number(r.total || 0).toLocaleString('de-CH',{minimumFractionDigits:2,maximumFractionDigits:2})}</td></tr>`).join("");
   const mHtml = validMat.map(r => `<tr><td style="padding:9px 12px">${r.name || "-"}</td><td style="text-align:right;font-family:monospace;padding:9px 12px;white-space:nowrap">${r.qty || 0} ${r.unit || ""}</td><td style="text-align:right;padding:9px 12px;white-space:nowrap">CHF ${Number(r.price || 0).toLocaleString('de-CH',{minimumFractionDigits:2,maximumFractionDigits:2})}</td><td style="text-align:right;font-family:monospace;padding:9px 12px;white-space:nowrap">CHF ${Number(r.total || 0).toLocaleString('de-CH',{minimumFractionDigits:2,maximumFractionDigits:2})}</td></tr>`).join("");
...
${mHtml ? `<div class="section-title">${tr.material}</div><table><thead><tr><th style="padding:9px 12px">${tr.description}</th><th style="text-align:right;padding:9px 12px">${tr.qty}</th><th style="text-align:right;padding:9px 12px">${tr.price}</th><th style="text-align:right;padding:9px 12px">Total</th></tr></thead><tbody>${mHtml}</tbody></table>` : ""}
