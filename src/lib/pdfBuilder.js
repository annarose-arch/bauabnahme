...
export function buildRechnungHtml({ language = "DE",
 ...
 const wHtml = validWork.map(r => `<tr><td>${r.employee || "-"}</td><td style="text-align:center">${r.from || "-"}–${r.to || "-"}</td><td style="text-align:right;font-family:monospace">${Number(r.hours || 0).toLocaleString('de-CH',{minimumFractionDigits:2,maximumFractionDigits:2})} h</td><td style="text-align:right;color:#333;width:35px">CHF</td><td style="text-align:right;font-family:monospace;width:90px">${Number(r.total || 0).toLocaleString('de-CH',{minimumFractionDigits:2,maximumFractionDigits:2})}</td></tr>`).join("");
   const mHtml = validMat.map(r => `<tr><td>${r.name || "-"}</td><td style="text-align:right;font-family:monospace;width:60px">${r.qty || 0}</td><td style="text-align:left;width:50px">${r.unit || ""}</td><td style="text-align:right;color:#333;width:35px">CHF</td><td style="text-align:right;font-family:monospace;width:90px">${Number(r.price || 0).toLocaleString('de-CH',{minimumFractionDigits:2,maximumFractionDigits:2})}</td><td style="text-align:right;color:#333;width:35px">CHF</td><td style="text-align:right;font-family:monospace;width:90px">${Number(r.total || 0).toLocaleString('de-CH',{minimumFractionDigits:2,maximumFractionDigits:2})}</td></tr>`).join("");
...
${mHtml ? `<div class="section-title">${tr.material}</div><table><thead><tr><th>${tr.description}</th><th style="text-align:right;width:60px">${tr.qty}</th><th style="text-align:left;width:50px">Einheit</th><th style="text-align:right;width:35px">CHF</th><th style="text-align:right;width:90px">${tr.price}</th><th style="text-align:right;width:35px">CHF</th><th style="text-align:right;width:90px">Total</th></tr></thead><tbody>${mHtml}</tbody></table>` : ""}
<div class="totals-box"><div class="totals-inner">
    ...
    ${discountPct > 0 ? `<tr><td style="padding:4px 0;font-size:13px">${tr.discount} ${discountPct}%</td><td style="padding:4px 0;text-align:right;width:40px;font-weight:700">CHF</td><td style="padding:4px 0;text-align:right;font-weight:700;font-size:13px;font-family:monospace;width:100px">−${Number(discountAmt).toLocaleString('de-CH',{minimumFractionDigits:2,maximumFractionDigits:2})}</td></tr>` : ""}
