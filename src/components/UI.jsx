...
export function SignaturePad({ value, onChange, clearLabel }) {
  ...
  useEffect(() => {
    ...
    if (!value) {
      ...
      const img = new Image();
      img.onload = () => { ctx.clearRect(0, 0, 300, 160); ctx.drawImage(img, 0, 0, 300, 160); };
      img.src = value;
    }
  }, [value]);
      ...
      {value ? (
        <div style={{ position: "relative" }}>
          <img
            src={value}
            alt={label}
            style={{ width: "100%", maxHeight: 200, objectFit: "cover", borderRadius: 8, border: `1px solid ${BORDER}` }}
          />
          <button
            type="button"
            onClick={() => onChange("")}
...
export function DemoBanner({ onNavigate, pBtn, gBtn }) {
  return (
    ...
    >
      ...
      <div style={{ display: "flex", gap: 8 }}>
        <button type="button" onClick={() => onNavigate("/login")} style={pBtn}>
          Registrieren
        </button>
        <button type="button" onClick={() => onNavigate("/login")} style={gBtn}>
          Zurück
        </button>
      </div>
    </div>
