...
export function HomeView({ customers = [], reports = [], archivedReports = [], invoices = [], offerten = [], onSelectCustomer, goTo, language = "DE", isDemo = false }) {
  const tr = useTranslation(language);
  const [search, setSearch] = useState("");
  const open = reports.filter(r => r.status === "offen" || r.status === "bearbeitet");
  const openOfferten = offerten.filter(o => o.status === "offen" || o.status === "bearbeitet");
...
        )}
      </SectionCard>
      <div style={{ display: "grid", gridTemplateColumns: window.innerWidth < 600 ? "1fr" : "1fr 1fr 1fr", gap: 12, margin: "12px 0" }}>
        <SectionCard>
          <div style={{ color: MUTED, fontSize: 12 }}>{tr.nav?.offerten || "Offerten"}</div>
          <div style={{ color: GOLD, fontWeight: 800, fontSize: 32, margin: "8px 0" }}>{openOfferten.length}</div>
          <button type="button" onClick={() => goTo("offerten")} style={gBtn}>{tr.common.show}</button>
        </SectionCard>
        <SectionCard>
...
// cache bust Fr  1 Mai 2026 12:33:15 CEST
