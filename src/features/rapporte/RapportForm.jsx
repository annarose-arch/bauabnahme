  ...
  return (
    <SectionCard>
      ...
      <div style={{ display: "grid", gap: 10 }}>
        ...
        <input placeholder="Projektname (optional)" value={reportForm.projectSearch || ""} onChange={e => setReportForm(p => ({ ...p, projectSearch: e.target.value }))} style={iStyle} />
          ...
          return (
            <div key={i} style={{ background: "rgba(255,255,255,0.03)", border: `1px solid ${BORDER}`, borderRadius: 8, padding: "10px 12px", marginBottom: 6 }}>
              <div style={{ display: "grid", gridTemplateColumns: "1fr auto", gap: 8, marginBottom: 8 }}>
                {employees.length > 0 && !isCustomEmp ? (
                  <select value={row.employee || ""} onChange={e => {
                    ...
                    {employees.map(emp => <option key={emp.id} value={emp.name}>{emp.name}{emp.rate ? ` — CHF ${emp.rate}/h` : ""}</option>)}
                  ...
                  <div style={{ display: "flex", gap: 6 }}>
                    ...
                    {employees.length > 0 && <button type="button" onClick={() => setWorkRows(p => p.map((r, j) => j === i ? { ...r, _customEmployee: false, employee: "" } : r))} style={{ ...gBtn, fontSize: 11, minHeight: 34, padding: "0 8px" }}>↩ Dropdown</button>}
                  </div>
                )}
          ...
          return (
            <div key={i} style={{ background: "rgba(255,255,255,0.03)", border: `1px solid ${BORDER}`, borderRadius: 8, padding: "10px 12px", marginBottom: 6 }}>
              <div style={{ display: "grid", gridTemplateColumns: "1fr auto", gap: 8, marginBottom: 6 }}>
                {materials.length > 0 && !isCustomMat ? (
                  <select value={row.name || ""} onChange={e => {
                    ...
                    {materials.map(mat => <option key={mat.id} value={mat.name}>{mat.name}{mat.unit ? ` (${mat.unit})` : ""}{mat.price ? ` — CHF ${mat.price}` : ""}</option>)}
                    <option value="__custom__">✏️ Manuell eingeben</option>
                  </select>
                ) : (
                  <div style={{ display: "flex", gap: 6 }}>
                    ...
                    {materials.length > 0 && <button type="button" onClick={() => setMaterialRows(p => p.map((r, j) => j === i ? { ...r, _customMaterial: false, name: "" } : r))} style={{ ...gBtn, fontSize: 11, minHeight: 34, padding: "0 8px" }}>↩ Dropdown</button>}
                  </div>
                )}
        ...
        <div style={{ display: "flex", gap: 8, position: "relative", zIndex: 30, marginTop: 4 }}>
          <button type="button" onClick={runSave} style={pBtn}>
            {editingReport ? "Änderungen speichern" : "Rapport speichern"}
          </button>
          {editingReport && (
