...
export function RechnungenView({ invoices, onReopen, onMarkSent, onMarkPaid, onDelete }) {
  return (
    <SectionCard>
      ...
      <div style={{ display: "grid", gap: 10 }}>
        {invoices.map(inv => {
          ...
          const paid = inv.status === "bezahlt";
          const sent = inv.status === "versendet";
          const borderColor = paid ? "#2d7a45" : sent ? GOLD : BORDER;
          return (
          <div key={inv.id} style={{ border: `1px solid ${borderColor}`, borderRadius: 10, padding: "12px 14px", background: "rgba(255,255,255,0.02)" }}>
            <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", flexWrap: "wrap", gap: 8, marginBottom: 8 }}>
              ...
              <div style={{ textAlign: "right", flexShrink: 0 }}>
                <span style={{
                  ...
                  background: paid ? "rgba(45,122,69,0.2)" : sent ? "rgba(212,168,83,0.15)" : "rgba(255,255,255,0.05)",
                  border: `1px solid ${paid ? "#2d7a45" : sent ? GOLD : BORDER}`,
                  color: paid ? "#7ddb9a" : sent ? GOLD : MUTED,
                }}>
                  {paid ? "Bezahlt ✓" : sent ? "✅ Versendet" : "📝 Entwurf"}
                </span>
              </div>
            </div>
            <div style={{ display: "flex", gap: 8, flexWrap: "wrap", borderTop: `1px solid ${BORDER}`, paddingTop: 8 }}>
              ...
              {inv.status === "versendet" && onMarkPaid && (
                <button type="button" onClick={() => onMarkPaid(inv)} style={{ ...gBtn, minHeight: 32, fontSize: 13, ...GREEN_BTN }}>
                  Bezahlt ✓
                </button>
              )}
