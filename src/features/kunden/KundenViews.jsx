  ...
  return (
    <SectionCard>
      ...
      <div style={{ display: "grid", gap: 8, marginBottom: 14 }}>
        ...
        <button type="button" onClick={onSave} style={pBtn}>Kunden speichern</button>
      </div>
      <div style={{ display: "grid", gap: 8 }}>
        {customers.map(c => (
          <div key={c.id} style={{ border: `1px solid ${BORDER}`, borderRadius: 10, padding: "10px 12px", display: "flex", justifyContent: "space-between", alignItems: "center" }}>
            <button type="button" onClick={() => onSelect(c)} style={{ border: "none", background: "transparent", color: TEXT, fontWeight: 700, cursor: "pointer" }}>{c.name}</button>
            <button type="button" onClick={() => onDelete(c)} style={dBtn}>Löschen</button>
          </div>
        ))}
      </div>
    </SectionCard>
  ...
  return (
    <SectionCard>
      ...
      <div style={{ display: "grid", gap: 4, marginBottom: 14 }}>
        ...
        <div><b>E-Mail:</b> {customer.email || "-"}</div>
      </div>
      ...
      <div style={{ display: "grid", gap: 8, marginBottom: 14 }}>
        {linked.map(r => {
          const rp = parseReport(r);
          const isArchived = r.status === "archiviert" || r.status === "gesendet";
          return (
            <div key={r.id} style={{ border: `1px solid ${isArchived ? GOLD : BORDER}`, borderRadius: 10, padding: "12px 14px", background: "rgba(255,255,255,0.02)" }}>
              <button type="button" onClick={() => onOpenReport(r)}
                ...
                <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", flexWrap: "wrap", gap: 6 }}>
                  ...
                  <div style={{ display: "flex", flexDirection: "column", alignItems: "flex-end", gap: 4 }}>
                    <span style={{ fontSize: 12, color: isArchived ? GOLD : MUTED, border: `1px solid ${isArchived ? GOLD : BORDER}`, borderRadius: 4, padding: "2px 8px", fontWeight: 700 }}>{r.status}</span>
              ...
              <div style={{ display: "flex", gap: 8, flexWrap: "wrap", borderTop: `1px solid ${BORDER}`, paddingTop: 8 }}>
                <button type="button" onClick={() => onEditReport(r)} style={{ ...gBtn, minHeight: 32, fontSize: 13 }}>✏️ Bearbeiten</button>
                <button type="button" onClick={() => onPDF(r)} style={{ ...gBtn, minHeight: 32, fontSize: 13 }}>🖨 PDF</button>
                <button type="button" onClick={() => onInvoice(r)} style={{ ...gBtn, minHeight: 32, fontSize: 13, color: "#7ddb9a", borderColor: "#2d7a45" }}>🧾 Rechnung</button>
                <button type="button" onClick={() => onDeleteReport(r)} style={{ ...dBtn, minHeight: 32, fontSize: 13 }}>🗑 Löschen</button>
              </div>
            </div>
          );
        })}
            ...
            <div key={inv.id} style={{ border: `1px solid ${inv.status === "versendet" ? GOLD : BORDER}`, borderRadius: 10, padding: "12px 14px", background: "rgba(255,255,255,0.02)" }}>
              <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 8 }}>
                ...
                <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
                  ...
                  <span style={{ fontSize: 11, padding: "2px 8px", borderRadius: 4, fontWeight: 700, border: `1px solid ${inv.status === "versendet" ? GOLD : BORDER}`, color: inv.status === "versendet" ? GOLD : MUTED }}>
                    {inv.status === "versendet" ? "✅ Versendet" : "📝 Entwurf"}
                  </span>
                </div>
              </div>
              <div style={{ display: "flex", gap: 8, flexWrap: "wrap", borderTop: `1px solid ${BORDER}`, paddingTop: 8 }}>
                <button type="button" onClick={() => onReopenInvoice(inv)} style={{ ...gBtn, minHeight: 32, fontSize: 13 }}>🖨 Öffnen</button>
                {inv.status === "entwurf" && <button type="button" onClick={() => onMarkInvoiceSent(inv)} style={{ ...pBtn, minHeight: 32, fontSize: 13 }}>✅ Versendet</button>}
                <button type="button" onClick={() => onDeleteInvoice(inv.id)} style={{ ...dBtn, minHeight: 32, fontSize: 13 }}>🗑</button>
              </div>
            </div>
          ))}
        </div>
